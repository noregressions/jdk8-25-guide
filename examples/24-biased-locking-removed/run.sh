...
#
# The reference doc's Symptom line says "No error -- a throughput regression."
# That's true for code that DOESN'T explicitly pass -XX:+UseBiasedLocking. But a
...
if $ok; then
  echo "REPRODUCED, but with a correction to the doc's framing: without the explicit flag, JDK 25 just runs (silent throughput-only difference, not tested here -- see README.md). WITH the flag explicitly carried forward from a JDK-8-era tuned config, JDK 25 refuses to start at all -- Won't Start, not Runs But Wrong, for anyone who set this flag on purpose."
  exit 0
else
