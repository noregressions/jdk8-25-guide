...
#
# This test investigates the reference doc's item #28 as literally worded --
...
if $ok; then
  echo
  echo "FINDING: evaluation order is IDENTICAL (1,2,3) on javac8/JDK8, javac25/JDK25, and the cross-version javac8-on-JDK25 run -- the claimed evaluation-order difference does NOT reproduce, confirming the pilot's earlier finding with a second, independent example (side-effecting toString(), not a char[]). The bytecode STRATEGY genuinely does change (StringBuilder chain -> invokedynamic), which is real and citable -- but changing HOW the JVM builds the string is not the same claim as changing WHEN side effects fire, and this test found no way to make the latter happen. Recommend softening the reference doc's item #28 wording to describe the bytecode-shape change only, and dropping the evaluation-order/side-effect claim unless a genuine reproducing example turns up."
  exit 0
else
