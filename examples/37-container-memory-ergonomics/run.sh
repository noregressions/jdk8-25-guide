...
# the 8u191 backport point where this was fixed. Both JDK 8 and
# JDK 25 correctly compute MaxHeapSize from the cgroup limit in this test.
# That's the EXPECTED result for these patch levels, not a test failure --
# it confirms the backport claim, but it means this sandbox CANNOT reproduce
# the actual "before" state this item describes (a pre-8u191
...
if [ ! -d /sys/fs/cgroup/memory ]; then
  echo "SKIP-REASON: no cgroup v1 memory controller (needs Linux, or Docker)"
...
mkdir -p "$CGROUP" 2>/dev/null || {
  echo "SKIP-REASON: cannot create a cgroup (needs root or cgroup delegation)"
...
echo $((512*1024*1024)) > "$CGROUP/memory.limit_in_bytes" 2>/dev/null || {
  echo "SKIP-REASON: cannot set a cgroup memory limit"
...
if [ -n "$j8_mb" ] && [ -n "$j25_mb" ] && [ "$j8_mb" -lt 536870912 ] && [ "$j25_mb" -lt 536870912 ]; then
  echo
  echo "CONFIRMED (not a failure -- see header comment): both JDK 8 (8u502, well past the 8u191 backport) and JDK 25 correctly compute MaxHeapSize from the 512MB cgroup limit rather than host RAM. This sandbox cannot reproduce the actual pre-8u191 broken behaviour -- that needs a genuinely old JDK 8 binary, not available here."
  exit 0
else
