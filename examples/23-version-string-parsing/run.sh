...
echo "$j25_out" | grep -q "extraction (split on first dot) = 25$" || ok=false
[ "$compile8_exit" -ne 0 ] || ok=false
echo "$compile8_out" | grep -q "cannot find symbol" || ok=false

if $ok; then
  echo "REPRODUCED: naive java.version parsing correctly (if fragilely) identifies '1' as the major version on JDK 8, and silently gets '25' -- not '1' -- on JDK 25 with the exact same code. The correct fix, Runtime.version(), doesn't even exist to fall back on if you're still compiling against JDK 8."
  exit 0
else
