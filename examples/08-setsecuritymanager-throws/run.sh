...
#
# Caveat: this sandbox only has JDK 8 and JDK 25 installed, so the JDK 18-23 "opt-out
# via =allow actually works" window in the reference doc's correction can't be
# empirically verified here -- only that by JDK 25 that window has definitely closed.
set -uo pipefail
