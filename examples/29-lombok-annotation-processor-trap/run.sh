...
#      when the caller directly references the generated methods (a harder,
#      more visible failure than the doc's "build succeeds silently" framing).
...
#      NoSuchMethodException. THIS is the scenario the doc's symptom line
#      describes -- it just isn't the only failure mode.
...
if $ok; then
  echo "REPRODUCED: implicit annotation-processor discovery works fine on JDK 8. On JDK 25 without an explicit -processorpath, Lombok's generated methods never materialize -- direct references fail to COMPILE (a harder failure than the doc implies), while reflective/framework-style calls compile fine and fail at RUNTIME with NoSuchMethodException, matching the doc's stated symptom exactly. Explicit -processorpath configuration fixes both."
  exit 0
else
