...
- `MainDirect.java` calls `p.setName(...)`/`p.getName()` directly — a compile-time
  ...
  **compile** ("cannot find symbol") — a harder, more visible failure than the
  usual "build succeeds silently" summary suggests.
- `MainReflective.java` looks up the same methods via `Class.getMethod(...)` —
  ...
  only fails at **runtime**, with `NoSuchMethodException` — this is the silent-build
  scenario, and it's real, but it needs the
...
identically without an explicit processor path on JDK 25 — so the Lombok version
requirement is about JDK-25-class-file *support* generally, not specifically about
this discovery-disabled behaviour.
...
**8–22** javac discovers annotation processors from the classpath implicitly · **23–26** discovery is off by default: generated methods vanish unless the processor path is explicit
