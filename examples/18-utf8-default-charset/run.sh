...
# (unforced) default always agree with themselves. A same-process round trip would
# therefore only ever reproduce on hosts whose locale happened to be non-UTF-8, which
# is why this test forces the writer's encoding explicitly instead (see NOTES.md).
#
