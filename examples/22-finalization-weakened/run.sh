...
# on borrowed time".
#
# The flag syntax is the trap here, and it is an easy one to fall into: the real
# flag is "--finalization=disabled" (a launcher option, double-dash, no -D), while
# the system-property spelling "-Dfinalization=disabled" parses fine and does
# nothing at all. Both are run below so the difference is visible -- verified
...
echo "JDK 25 with -Dfinalization=disabled (the system-property spelling, as a control -- should NOT disable it):"
...
if $ok; then
  echo "REPRODUCED: finalize() still runs identically on JDK 8 and JDK 25 today. --finalization=disabled (launcher option) turns it off entirely with a one-flag change; -Dfinalization=disabled (the system-property spelling) has NO effect at all, parsing cleanly and doing nothing, which makes it the more dangerous of the two spellings to reach for."
  exit 0
else
